  ...
  var ColorPicker = (function () {
    var ids = {},
      ...
      moveIncrement = function (ev) {
        ev.data.field.val(
          Math.max(0, Math.min(ev.data.max, parseInt(ev.data.val + ev.pageY - ev.data.y, 10)))
        )
        if (ev.data.preview) {
      ...
      moveHue = function (ev) {
        change.apply(
          ev.data.cal
            ...
            .fields.eq(4)
            .val(
              parseInt((360 * (150 - Math.max(0, Math.min(150, ev.pageY - ev.data.y)))) / 150, 10)
            )
            .get(0),
          [ev.data.preview]
        )
      ...
      moveSelector = function (ev) {
        change.apply(
          ev.data.cal
            ...
            .val(
              parseInt(
                (100 * (150 - Math.max(0, Math.min(150, ev.pageY - ev.data.pos.top)))) / 150,
                10
              )
            )
            .end()
            .eq(5)
            .val(
              parseInt((100 * Math.max(0, Math.min(150, ev.pageX - ev.data.pos.left))) / 150, 10)
            )
            .get(0),
          [ev.data.preview]
        )
      ...
      clickSubmit = function (ev) {
        ...
        cal
          .data('colorpicker')
          .onSubmit(col, HSBToHex(col), HSBToRGB(col), cal.data('colorpicker').el)
      },
      show = function (ev) {
      ...
      getViewport = function () {
        ...
        return {
          ...
          h:
            window.innerHeight ||
            (m ? document.documentElement.clientHeight : document.body.clientHeight),
        }
      },
      fixHSB = function (hsb) {
        return {
